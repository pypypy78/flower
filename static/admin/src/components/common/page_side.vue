        ...
        @click="handleButtonClick(` ${i.mod_name}管理`)"><span>{{ i.page_title + "列表" }}</span></el-menu-item>
         <el-menu-item  v-if="i.has_add && (user_group == '管理员' || $check_action(i.path.replace(/(.*)table/,'$1view'), 'add'))"  :index="i.path.replace(/(.*)table/,'$1view')" :class="{ sactive: $route.path === i.path }"
          ...
          <template slot="title"><i class="el-icon-shopping-cart-2"></i><span ref="customTemplate7">商城管理</span></template>
                                                                          <el-menu-item v-show="user_group == '管理员' || $check_action('/flower_mall/table')"
                                index="/flower_mall/table"
                                class="bg-hover" @click="handleButtonClick(7)"><span>鲜花商城</span></el-menu-item>
  ...
  export default {
    ...
    computed: {
      auth() {
        ...
        var lt = [];
        let site_list = []
                                                                                    site_list.push("storage_information")
                                            let list = ["exam", "article", "article_type", "comment", "goods", "order", "logistics_delivery", "notice"];
                                                                  list.push("flower_mall");
