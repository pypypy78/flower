	...
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
                ...
                <el-form-item label="分类" prop="cart_type">
                    <el-select v-model="form.cart_type">
                        <el-option v-for="(o ,i) in list_cart_goods_type" :value="o.name" :key="i"
							...
							<el-col v-if="user_group === '管理员' || $check_field('get','flower_date') || $check_field('add','flower_date') || $check_field('set','flower_date')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="鲜花日期" prop="flower_date">
								<el-date-picker :disabled="disabledObj['flower_date_isDisabled']" v-if="user_group === '管理员' || (form['flower_mall_id'] && $check_field('set','flower_date')) || (!form['flower_mall_id'] && $check_field('add','flower_date'))" id="flower_date"
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/flower_mall/view','set') || $check_action('/flower_mall/view','add') || $check_option('/flower_mall/table','examine')">
	...
	export default {
		...
		methods: {
			...
			get_obj_before(param) {
		        ...
		        if (this.form["flower_date"] && this.form["flower_date"].indexOf("-")===-1){
          this.form["flower_date"] = this.$toTime(parseInt(this.form["flower_date"]),"yyyy-MM-dd")
        }
					$.db.del("form");
			...
			get_obj_after(json, func){
						if(this.form["flower_date"]=="0000-00-00"){
				  this.form["flower_date"] = null;
				}
				if(parseInt(this.form["flower_date"]) > 9999){
					this.form["flower_date"] = this.$toTime(parseInt(this.form["flower_date"]),"yyyy-MM-dd")
				}
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/flower_mall/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/flower_mall/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/flower_mall/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/flower_mall/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/flower_mall/view','get');
            ...
            submit_after(){
                ...
                if (this.form[this.field]){
                    ...
                    this.$get("~/api/flower_mall/get_obj?",query_form,(res)=>{
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
							customize_field.push({"field_name":"鲜花日期","field_value":this.form.flower_date,"type":"date"});
			                this.goods_form = {
                    goods_id: 0,
                ...
                if (add_flag==='set'){
                    ...
                    // 提交事件
                    this.$post("~/api/goods/set?source_table=flower_mall&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
