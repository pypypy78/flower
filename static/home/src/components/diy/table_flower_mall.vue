  ...
  <el-main class="bg table_wrap">
    <el-form label-position="right" :model="query" class="form p_4" label-width="120">
              ...
              <el-form-item label="分类">
                <el-select v-model="query.cart_type">
                  ...
                  <el-option v-for="(o,i) in list_goods_cart_type" :key="i" :value="o.name" :label="o.name"></el-option>
                                                              ...
                                                              <el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
                      <el-form-item label="鲜花日期">
                                                      <el-input v-model="query.flower_date"></el-input>
                                                </el-form-item>
                    </el-col>
                                                                </el-row>
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
            ...
            <el-button v-if="user_group == '管理员' || $check_action('/flower_mall/table','add') || $check_action('/flower_mall/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
						
            <el-button v-if="user_group == '管理员' || $check_action('/flower_mall/table','del') || $check_action('/flower_mall/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
    ...
    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
          ...
          <el-table-column prop="cart_description" label="描述" min-width="200">
                                          ...
                                          <el-table-column prop="flower_date" @sort-change="$sortChange" label="鲜花日期"
                               v-if="user_group == '管理员' || $check_field('get','flower_date')" min-width="200">
                                      <template slot-scope="scope">
                      {{ $toTime(scope.row["flower_date"],"yyyy-MM-dd") }}
                    </template>
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/flower_mall/table','set') || $check_action('/flower_mall/view','set') || $check_action('/flower_mall/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="user_group == '管理员' || $check_action('/flower_mall/table','set') || $check_action('/flower_mall/view','set') || $check_action('/flower_mall/view','get')"
                                 :to="'./view?' + field + '=' + scope.row[field]"
                                 size="small">
          <span>详情</span>
        </router-link>
                                                                      <router-link v-if="user_group == '管理员' || $check_comment('/flower_mall/table')" class="el-button el-button--small is-plain el-button--primary"
